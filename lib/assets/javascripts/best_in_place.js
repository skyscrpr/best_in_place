...
BestInPlaceEditor.prototype = {
  ...
  update : function() {
    var editor = this;
    var oldValue = "";
    var wrapped_inside_sky_div = this.element.data('wrapped-inside-sky-div-class');
    // Super naive way of doing this... It's horrible.
    if (wrapped_inside_sky_div) {
      oldValue = $(this.oldValue).html();
    }
    else {
      oldValue = this.oldValue;
    }
    //
    if (this.formType in {"input":1, "textarea":1} && this.getValue() == oldValue)
    ...
    if (this.formType == "select") {
      ...
      if (this.getValue() !== "") {
        editor.element.html(this.skyDivValueDecorator(this.getValue()));
      } else {
        editor.element.html(this.skyDivValueDecorator(this.nil));
      }
    }
  ...
  skyDivValueDecorator: function(value) {
    var wrappedInsideSkyDivClass = this.element.data('wrapped-inside-sky-div-class');
    if (wrappedInsideSkyDivClass) {
      thisValue = "<div class='" + wrappedInsideSkyDivClass + "'>" + value + "</div>";
    }
    else {
      thisValue = value;
    }
  ...
  initNil: function() {
    ...
    {
      this.element.html(this.skyDivValueDecorator(this.nil));
    }
  },
...
BestInPlaceEditor.forms = {
  ...
  "select" : {
    activateForm : function() {
      ...
      this.element.html(output);
      this.setHtmlAttributes();
      this.element.find("select").bind('change', {editor: this}, BestInPlaceEditor.forms.select.changeHandler);
